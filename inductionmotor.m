...
%  picture
ax4=axes('Position',[0.35  .22  .2  .2]);
[x,map] = imread('IM001','jpg');
image(x)
set(gca,'visible','off')
...
yd1=[1.2  1.2];
plot(xd1, yd1, 'b') 
text(-.835,1.1625, '\circ', 'color', [0 0 0])
text(-.835,-.0375, '\circ', 'color', [0 0 0])
text(-.8, .6, 'V_{1\phi}', 'Fontsize', 8, 'color', [0 0 0])
text(1.6, .35, 'X_{m}', 'Fontsize', 8, 'color', [0 0 0])
text(.9, .35, 'R_{c}', 'Fontsize', 8, 'color', [0 0 0])
text(-.34, .975, 'R_1', 'Fontsize', 8, 'color', [0 0 0])
text(.6, 1.05, 'X_1', 'Fontsize', 8, 'color', [0 0 0])
text(1.85, 1, 'R_2^\prime', 'Fontsize', 8, 'color', [0 0 0])
text(2.9, 1.05, 'X_2\prime', 'Fontsize', 8, 'color', [0 0 0])
text(3.7, .6, 'R_L^\prime= {R_2}^\prime(1-s)/s', 'Fontsize', 8, 'color', [0 0 0])

text(-.2, -.1, 'Equivalent Circuit per phase referred to the Stator', 'color',[0 0 0])
%%%%%%%%
hold off
...
text(-.835,1.1625, '\circ', 'color', [0 0 0])
text(-.835,-.0375, '\circ', 'color', [0 0 0])
%text(1.6, .35, 'jX_{m}', 'Fontsize', 10, 'color', [0 0 0])
%text(.9, .35, 'R_{c}', 'Fontsize', 10, 'color', [0 0 0])
text(-.34, .975, 'R_{th}', 'Fontsize', 10, 'color', [0 0 0])
text(.6, 1.05, 'jX_{th}', 'Fontsize', 10, 'color', [0 0 0])
text(1.85, 1, 'R_2^\prime', 'Fontsize', 10, 'color', [0 0 0])
text(2.9, 1.05, 'jX_2\prime', 'Fontsize', 10, 'color', [0 0 0])


text(-.2, -.15, 'Thevenin''s Equivalent Circuit per phase', 'color',[0 0 0])
text(-.5, 1.4, ['Z_{th} = ', num2str(Rth),' + j', num2str(Xth), ' \Omega'], 'Fontsize', 10, 'color', [0 0 0])
text(1.7, 1.4, ['{Z_2}^\prime = ', num2str(R2) ' + j', num2str(abs(X2)), ' \Omega'], 'Fontsize', 10, 'color', [0 0 0])
text(3.7, .75, 'R_L^\prime= {R_2}^\prime(1-s)/s', 'Fontsize', 10, 'color', [0 0 0])
text(3.7, .45, ['{R_L}^\prime = ', num2str(RLd), ' \Omega'], 'Fontsize', 10, 'color', [0 0 0])
text(-1.2, .6, ['V_{\phi (th)} = ', num2str(Vth),'\angle', num2str(Tth) ' V'], 'Fontsize', 10, 'color', [0 0 0])

text(3.2, 1.4, '{I_2}^\prime \rightarrow', 'color', [0 0 0])
text(-.7, 1.7,['\bfAnalysis of the Equivalent Circuit at Slip s = ',num2str(S)], 'color', [0  0  0]) 
hold off
...
text(0,.7, ['P_{in} = ', num2str(Pin), ' W'], 'Fontsize', 10,'color', [0 0 0 ]); 
text(0,.6, ['P_{cu (stator)} = ', num2str(SCL), ' W'], 'Fontsize', 10,'color', [0 0 0 ]); 
text(0,.5, ['P_{core (stator)}  = ', num2str(Pcore), ' W'], 'Fontsize', 10,'color', [0 0 0 ]); 
text(0,.4, ['P_{ag} = ', num2str(Pg), ' W'], 'Fontsize', 10,'color', [0 0 0 ]); 
text(0,.3, ['P_{cu (rotor)} = ', num2str(RCL), ' W'], 'Fontsize', 10,'color', [0 0 0 ]) 
text(0,.2, ['T_d = ', num2str(Td), ' N-m'], 'Fontsize', 10,'color', [0 0 0 ]); 
if S~=1
text(0,.1, ['P_m = ', num2str(Pm), ' W'], 'Fontsize', 10,'color', [0 0 0 ]); 
text(0, 0, ['P_{rot} = ', num2str(Prot), ' W'], 'Fontsize', 10,'color', [0 0 0 ]); 
text(0, -.1, ['P_o = ', num2str(Po), ' W,  ', num2str(hp), ' hp'], 'Fontsize', 10,'color', [0 0 0 ]); 
text(0,-.2, ['T_o = ', num2str(To), ' N-m'], 'Fontsize', 10,'color', [0 0 0 ]); 
text(0,-.3 ,['\eta = ', num2str(Ef), ' %'], 'Fontsize', 10,'color', [0 0 0 ]); 
%%
...
xlabel('Rotor speed rpm', 'color', [0 0 0])
ylabel('Developed Torque N-m' , 'color', [0 0 0])
title('Motor torque-speed Graph', 'color', [0 0 0])


if Nr ~= 0
  text(x3, y3, 'T_d', 'color',[0 0 0])
else, end
if S~=1
if S >= Smax
   text(x1,.75*y1, ['s_{max} = ',num2str(Smax), ', s \geq s_{max}'], 'color', [0 0 0])
   text(x1, .5*y1,'Motor cannot operate in this region.', 'color', [0 0 0])
   else, end
   hold off, 
else, end
